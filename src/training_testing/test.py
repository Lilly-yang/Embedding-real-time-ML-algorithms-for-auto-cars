...
import argparse
# from Image_Subscriber import image_converter
...
train_data = np.load('MUSHR_320x240_shuffled_Steering_test.npy'.format(args.dataset_name),allow_pickle=True)
...
with torch.no_grad():
    for j in range(0,len(X),10):
        ...
        time.sleep(0.001)
        img = torch.Tensor(img).view(-1,1,800,848).to(device)
        # print(img.dtype)
        now = time.time()
        output = model(img)[0]
        delta = time.time()-now

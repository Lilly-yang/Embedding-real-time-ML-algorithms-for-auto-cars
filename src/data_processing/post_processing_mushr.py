...
def create_steering_data(args):
    ...
    data_path = '/home/li/Documents/sensor_data/data/0702183751/tmp_data'
    for file_name in os.listdir(data_path):
        ...
        ## seperate stree to left and right
        s_c = float(file_name.split('_')[-1][:5])*30-15
        l_c,r_c = get_lr(s_c)

        output_c.append([img, np.array([l_c,r_c])])

    final_out = np.array(output_c)
    np.random.shuffle(final_out)
    print(final_out.shape)
    np.save('/home/li/Documents/sensor_data/src/data_processing/data/MUSHR_320x240_shuffled_Steering_{}.npy'.format(args.dataset_name), final_out)
    del output_c
...
def create_bezier_data(args):
    ...
    np.random.shuffle(final_out)
    print(final_out.shape)
    np.save('/home/li/Documents/sensor_data/src/data_processing/data/MUSHR_320x240_shuffled_Bezier_{}.npy'.format(args.dataset_name), final_out)
    print('save data to %s'%'/home/li/Documents/sensor_data/src/data_processing/data/MUSHR_320x240_shuffled_Bezier_{}.npy'.format(args.dataset_name))
